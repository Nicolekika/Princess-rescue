(() => {
  ...
  setTimeout(()=>dlgStart.showModal(),40);
  btnStart?.addEventListener('click',()=>{
    if(localStorage.getItem('age_ok')!=='yes') setTimeout(()=>dlgAge.showModal(),30);
  });
  enter?.addEventListener('click',(e)=>{
    if(!confirm18.checked){ e.preventDefault(); alert('Bekräfta 18+.'); }
    else localStorage.setItem('age_ok','yes');
  });

  let ac, musicOn=true, timer;
  function initAudio(){ if(ac) return; ac=new (window.AudioContext||window.webkitAudioContext)(); startMusic(); }
  function beep(f=440,d=0.12,type='square',g=0.25){ if(!ac) return; const o=ac.createOscillator(), gain=ac.createGain(); o.type=type; o.frequency.value=f; gain.gain.value=g; o.connect(gain); gain.connect(ac.destination); o.start(); gain.gain.exponentialRampToValueAtTime(0.001, ac.currentTime+d); o.stop(ac.currentTime+d+0.02); }
  function startMusic(){ if(!ac||!musicOn) return; stopMusic(); const seq=[262,330,392,523,392,330,294,330]; let i=0; timer=setInterval(()=>{beep(seq[i%seq.length],0.15,'triangle',0.14); i++;},320); }
  ...
  function bind(btn,down,up){ const d=(e)=>{e.preventDefault();down();}; const u=(e)=>{e.preventDefault();up();}; btn.addEventListener('touchstart',d);btn.addEventListener('mousedown',d);btn.addEventListener('touchend',u);btn.addEventListener('mouseup',u);btn.addEventListener('mouseleave',u);}
  ...
  function update(dt){
    ...
    if(jump && player.on){player.vy=-JUMP; player.on=false; beep(660,0.08,'square',0.25);}
    ...
    for(const h of hearts){if(!h.t){const dx=(player.x+player.w/2)-h.x,dy=(player.y+player.h/2)-h.y; if(Math.hypot(dx,dy)<20){h.t=true;got++;heartsEl.textContent=got; beep(880,0.12,'triangle',0.2);} }}
  ...
  function draw(){
    ctx.fillStyle='#ffe7f2'; ctx.fillRect(0,0,W,H);
    ...
    ctx.fillStyle='#f6a0c1'; for(const s of spikes){const[x,y,size]=s; ctx.beginPath(); ctx.moveTo(x,y); ctx.lineTo(x+size,y); ctx.lineTo(x+size/2,y-size); ctx.closePath(); ctx.fill();}
    ctx.fillStyle='#c6a3ff'; for(const b of blobs) ctx.fillRect(b.x,b.y,b.w,b.h);
    for(const h of hearts){ if(h.t) continue; ctx.fillStyle='#ff7aa2'; ctx.fillRect(h.x-2,h.y,4,4); ctx.fillRect(h.x-4,h.y+2,8,4); ctx.fillRect(h.x-6,h.y+4,12,4); ctx.fillRect(h.x-4,h.y+8,8,4); ctx.fillRect(h.x-2,h.y+10,4,4); }
